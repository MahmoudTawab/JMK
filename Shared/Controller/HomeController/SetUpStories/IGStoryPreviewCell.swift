...
final class IGStoryPreviewCell: UICollectionViewCell, UIScrollViewDelegate {
    ...
    var PreviewController : IGStoryPreviewController?
                ...
                case .forward:
                    if snapIndex < story?.snapsCount ?? 0 {
                        if let snap = story?.snaps[snapIndex] {
                            ...
                            storyHeaderView.lastUpdatedLabel.text = snap.Text
                        }
                }
                case .backward:
                    if snapIndex < story?.snapsCount ?? 0 {
    ...
    override init(frame: CGRect) {
        ...
        ShareButton.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: ControlY(-15)).isActive = true
        ShareButton.rightAnchor.constraint(equalTo: self.rightAnchor, constant: ControlX(-15)).isActive = true
        ShareButton.widthAnchor.constraint(equalToConstant: ControlWidth(40)).isActive = true
        ShareButton.heightAnchor.constraint(equalToConstant: ControlHeight(40)).isActive = true
    }
    ...
    lazy var ShareButton: UIButton = {
        ...
        Button.tintColor = #colorLiteral(red: 0.8775765133, green: 0.7798625896, blue: 0.7378370855, alpha: 1)
    ...
    @objc func share() {
    let objectsToShare = [getSnapview()?.image ?? UIImage(),"JMK App", ""] as [Any]
    let activityViewController = UIActivityViewController(activityItems: objectsToShare, applicationActivities: nil)
    activityViewController.popoverPresentationController?.sourceView = PreviewController?.view
    activityViewController.view.tintColor = UIColor(red: 99/255, green: 87/255, blue: 82/255, alpha: 1)
    PreviewController?.present(activityViewController, animated: true) { [weak self] in
    ...
    static let StoryNumber = NSNotification.Name(rawValue: "story")
    private func gearupTheProgressors(type: MimeType, playerView: IGPlayerView? = nil) {
        
        if snapIndex == 0 {
            NotificationCenter.default.post(name: IGStoryPreviewCell.StoryNumber, object: nil)
        }
        ...
        if let holderView = getProgressIndicatorView(with: snapIndex),
            ...
            DispatchQueue.main.async {
                if type == .image {
                progressView.start(with: TimeInterval(self.story?.snaps[self.snapIndex].timer ?? 5), holderView: holderView, completion: {(identifier, snapIndex, isCancelledAbruptly) in
...
//        story?.snaps[snapIndex].isDeleted = true
    ...
    public func startSnapProgress(with sIndex: Int) {
        if let indicatorView = getProgressIndicatorView(with: sIndex),
            ...
            pv.start(with: TimeInterval(self.story?.snaps[self.snapIndex].timer ?? 5), holderView: indicatorView, completion: { (identifier, snapIndex, isCancelledAbruptly) in
                if isCancelledAbruptly == false {
                    self.didCompleteProgress()
                }
            })
